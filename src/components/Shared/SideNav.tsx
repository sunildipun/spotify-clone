...
const SideNav = () => {
    ...
    return (
        ...
        <aside className= {`h-full text-slate-50 text-xl ${collapsed ? 'w-14 overflow-hidden' : 'basis-1/4'}`}>
            <div className="flex flex-col gap-2">
                ...
                <ul className="flex flex-col gap-2 bg-neutral-900 rounded py-3 px-5">
                    <li className="h-12 flex items-center" onClick={() => setCollapsed(!collapsed)}>
                        <span className="mr-4">{collapsed ? <FaWindowMaximize/> : <FaWindowMinimize />}</span>Expand/Collapse Playlist
                    </li>
